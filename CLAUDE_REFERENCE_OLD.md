...
## ⚡ OPERATIONAL RULE — launching/testing is a SOLVED one-shot (don't overthink)

"Launch / run / test svcldb", "put it on my screen", "dev bypass then launch" =
run the known procedure immediately. Do NOT burn turns on recon (no deploy-dir
listings, no reading `build.bat`s, no grepping `main.c` for flags, no source
diffs) just to launch. For "testing" default to **dev bypass + `--reinject`**:
...
pass with `tools\dlog.ps1 -Tail 40` (`HANDSHAKE SKIPPED` + `hooks_install: SUCCESS`
+ `get_backbuffer_texture: OK` + `ImGui READY`). Overlay force-shows on inject.
Full detail: `.cursor/rules/fast-testing-launch.mdc` + `AGENTS.md`.
